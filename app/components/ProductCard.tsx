...
const ProductCard = ({
    ...
    const { category, image: { desktop: imgSrc }, price, name: title } = productData;

    return (
        <div id="product_card" className="flex flex-col gap-[calc(var(--spacing-200)+22px)]">
            <div className="relative">
                <Image src={imgSrc} alt={title} width={327} height={240} className="rounded-(--spacing-100)" />
            ...
            <div className="grid gap-(--spacing-50)">
                <span className="text-preset-4 text-(--color-rose-500)">{category}</span>
                <h5 className="text-preset-3 text-(--color-rose-900)">{title}</h5>
